...
impl<T: TableSchema> QueryConverter for CKLogConverter<T> {
    fn convert_condition(&self, c: &Condition) -> String {
		...
		match &c.cmp {
			...
			Cmp::RegexNotMatch(v) => format!("NOT match({}, '{}')", col_name, v),
			Cmp::Contains(v) => {
				v.split(" ").map(|s| format!("hasToken({}, '{}')", col_name, s)).collect_vec().join(" AND ")
			},
			Cmp::NotContains(v) => {
				v.split(" ").map(|s| format!("NOT hasToken({}, '{}')", col_name, s)).collect_vec().join(" AND ")
			}
		}
    }
