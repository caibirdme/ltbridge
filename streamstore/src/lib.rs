...
impl Labels {
    ...
    fn matches(&self, conditions: &HashMap<String, String>) -> bool {
        conditions.iter().all(|(k, v)| {
            self.0.get(k).map_or(false, |value| value == v)
        })
    }
}
...
mod tests {
    ...
    fn test_query_multiple_conditions() {
        let store = StreamStore::new();
        let records = vec![
            create_labels(&[("env", "prod"), ("service", "api"), ("region", "us-east")]),
            create_labels(&[("env", "prod"), ("service", "web"), ("region", "us-east")]),
            create_labels(&[("env", "dev"), ("service", "api"), ("region", "eu-west")]),
        ];
    ...
    fn test_large_dataset() {
        ...
        let useast_records = store.query(create_labels(&[("region", "us-east")]));
